...
fn is_range(cx: &LateContext<'_>, qpath: &QPath, hir_id: HirId) -> bool {
    [
        LangItem::Range,
    ...
    .into_iter()
    .any(|item| is_lang_item(cx, qpath, hir_id, item))
}
...
fn handle_expr<'a>(me: &mut ForLoopVisitor<'a, '_, '_>, expr: &'a Expr<'a>) -> Option<()> {
    ...
    if !is_lang_item(me.cx, &qpath, func.hir_id, LangItem::IntoIterIntoIter) {
        return Some(());
    }
    if args.first().is_none() {
        return Some(());
    }
    let range_expr = args.first().unwrap();
    let (qpath, fields, _) = expr_to_struct(&range_expr.kind)?;
    if !is_range(me.cx, qpath, range_expr.hir_id) {
        return Some(());
    }
    ...
    for arm in some_none_arms {
        let hir_id = (|| -> Option<HirId> {
            ...
            if !is_lang_item(me.cx, qpath, arm.pat.hir_id, LangItem::OptionSome) {
                return None;
            }
