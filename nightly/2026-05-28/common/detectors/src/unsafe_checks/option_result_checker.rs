...
impl CheckType {
    ...
    fn from_pattern(pat: &Pat<'_>) -> Option<Self> {
        match &pat.kind {
            ...
            PatKind::Ref(inner, _, _) => Self::from_pattern(inner),
...
impl<'a, 'tcx> OptionResultChecker<'a, 'tcx> {
    ...
    pub fn check_for_try(&mut self, expr: &Expr<'tcx>) {
        if_chain! {
            ...
            if self.cx.qpath_res(qpath, func.hir_id).opt_def_id()
                .is_some_and(|did| clippy_utils::is_lang_item_or_ctor(self.cx, did, LangItem::TryTraitBranch));
