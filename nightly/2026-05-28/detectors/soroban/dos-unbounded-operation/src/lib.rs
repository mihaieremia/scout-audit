...
use common::{
    analysis::{get_node_type_opt, is_lang_item, is_soroban_map, is_soroban_vec},
...
impl<'tcx> Visitor<'tcx> for ForLoopVisitor<'tcx, '_> {
    fn visit_expr(&mut self, expr: &'tcx Expr<'tcx>) {
        ...
        if_chain! {
            ...
            if is_lang_item(self.cx, qpath, call_func.hir_id, LangItem::IntoIterIntoIter);
            // Check if a Range is used
            if let Some(range_arg) = call_args.first();
            if let ExprKind::Struct(struct_lang_item, struct_expr, _) = range_arg.kind;
            if is_lang_item(self.cx, struct_lang_item, range_arg.hir_id, LangItem::Range)
                || is_lang_item(self.cx, struct_lang_item, range_arg.hir_id, LangItem::RangeInclusiveStruct)
                || is_lang_item(self.cx, struct_lang_item, range_arg.hir_id, LangItem::RangeInclusiveNew);
